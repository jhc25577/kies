...
# TODO: change this to work with Kyber key
# actually, it might already work
def sym_encrypt(key: bytes, plain_text: bytes) -> bytes:
    ...
    """
    algorithm = KIES_CONFIG.symmetric_algorithm
    if algorithm == "aes-256-gcm":
        nonce_length = KIES_CONFIG.symmetric_nonce_length
        nonce = os.urandom(nonce_length)
        cipher = AES.new(derive_key(key), AES_CIPHER_MODE, nonce)
    elif algorithm == "xchacha20":
        nonce = os.urandom(XCHACHA20_NONCE_LENGTH)
        cipher = ChaCha20_Poly1305.new(key=derive_key(key), nonce=nonce)  # type:ignore
    else:
...
def sym_decrypt(key: bytes, cipher_text: bytes) -> bytes:
    ...
    # You can configure to use 12 bytes nonce by setting `KIES_CONFIG.symmetric_nonce_length = 12`
...
# TODO: Use this function inside of the sym_encrypt and sym_decrypt to derive suitable keys for AES from Kyber
